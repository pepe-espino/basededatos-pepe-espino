...
SELECT e.*, d.* FROM empleado e JOIN departamento d ON e.codigo = d.codigo;
...
ORDER BY d.nombre, e.primer_apellido, e.segundo_apellido, e.nombre;
...
SELECT d.nombre FROM departamento d 
JOIN empleado e ON d.codigo = e.codigo 
WHERE e.nombre = 'Pepe' AND e.primer_apellido = 'Ruiz' AND e.segundo_apellido = 'Santana';
...
SELECT * FROM departamento WHERE codigo NOT IN (SELECT DISTINCT codigo FROM empleado WHERE codigo IS NOT NULL);
...
SELECT e.*, d.* FROM empleado e 
RIGHT JOIN departamento d ON e.codigo = d.codigo
ORDER BY d.nombre;
